...
#include "../../accel-sim-dev/util/tuner/GPU_Microbenchmark/hw_def/hw_def.h"
...
template <class T, class R> float tensor_lat(int repeat_iters) {
  ...
  tensor_latency<T, R><<<BLOCKS_NUM, THREADS_PER_BLOCK>>>(
      startClk_g, stopClk_g, data1_g, data2_g, res_g, repeat_iters);
  ...
  std::cout << "Number of repeat iters = " << repeat_iters << "\n";
